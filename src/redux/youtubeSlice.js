...
//비동기 서버통신으로 데이터를 전달 받아서 내부적으로 action타입을 자동생성해서 액션객체 생성까지 완료하는 함수
...
/*
    store: 전역 state 저장공간 (은행금고 개념)
    reducer: state의 전역 state값을 변경해주는 함수 (변형자) (금고관리인, 점장 개념)
    dispatch: reducer에 데이터 변경요청을 해주는 함수 (전달자) (창구직원)
    action: dispatch로 리듀서에 데이터 변경요청을 위해 필요한 특별한 형태의 객체 (창구직원에게 전달하기 위한 양식)
    action객체의 구조 : {type, payload}

    1 --컴포넌트에서 데이터 변경이나 데이터 요청을 위한 action객체를 만들어서 dispatch함수에 전달
    ...
    5 --루트 컴포넌트인 App.jsx에 데이터가 전달되었기 때문에 자식 컴포넌트 어디에서는 해당 데이터에 접근가능
